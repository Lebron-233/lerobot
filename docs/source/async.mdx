...
<Tip>
  This guide describes the networked `PolicyServer`/`RobotClient` system. The
  experimental SmolVLA `--inference.type=predictive_async` backend instead runs
  a latency-planned worker and scheduled queue inside `lerobot-rollout`; see
  [Policy Deployment](./inference#predictive-async-for-smolvla).
</Tip>

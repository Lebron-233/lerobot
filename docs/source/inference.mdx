...
PR-1 provides live `identity` context: it reuses the current model-ready image
tokens and state to exercise the scheduled takeover contract, without claiming
learned future prediction. `oracle` is reserved for an explicit offline/test
provider and is rejected by live rollout. `predicted` is not implemented yet.
...
| `--inference.max_prediction_delay`      | Largest future horizon supported by the predictor contract (default: 8)                       |
...
| `--inference.context_mode`              | Live PR-1 supports `identity`; `oracle` is offline/test-only and `predicted` is not available |

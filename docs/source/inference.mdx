...
| `--inference.future_latent_checkpoint`  | Frozen B3.3a best checkpoint; required for `predicted`, invalid with `identity` (default: None) |
...
--inference.future_latent_checkpoint=/home/rp/Workspace/SmolVLA_RTC/artifacts/b33a_9e618076f_lambda-risk-005_v1/best.pt
--inference.min_prediction_delay=1
--inference.max_prediction_delay=8
```

The fragment omits `--robot.max_relative_target`; the resulting configuration
...
This is an implementation-only candidate. Synthetic contract checks do not
establish actual-checkpoint integration, online predicted performance, or
robot behavior. Real checkpoint/model loading, offline integration smoke,
online rollout, and robot connection or motion require a separate approved
stage. The CLI fragment is not a verified robot command. The candidate's
